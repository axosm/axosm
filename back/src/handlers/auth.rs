...
pub async fn register(
    ...
    let token = make_token(player_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(AuthResponse { token, player_id, username: req.username }))
}
...
pub async fn login(
    ...
    let token = make_token(player.id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(AuthResponse { token, player_id: player.id, username: player.username }))
}
...
fn make_token(player_id: i64) -> anyhow::Result<String> {
    ...
    let claims = Claims { sub: player_id, exp };
    let token = encode(
        &Header::default(),
        &claims,
